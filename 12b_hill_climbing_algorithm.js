const char = (letter) => letter.charCodeAt(0)
const dirs = [
    [0, 1],
    [0, -1],
    [1, 0],
    [-1, 0],
]

function can_go_from_A_to_B(x_a, y_a, x_b, y_b, map) {
    return map[y_a][x_a] + 1 >= map[y_b][x_b]
}

function inbounds(x, y, map) {
    return x >= 0 && y >= 0 && x < map[0].length && y < map.length
}


function calculate_pos(x, y, map, length_map) {
    let min = length_map[y][x];

    for (let [dx, dy] of dirs) {
        dx = Number.parseInt(dx)
        dy = Number.parseInt(dy)

        if (inbounds(x + dx, y + dy, map)) {
            let neighbour = length_map[y + dy][x + dx]
            if (neighbour != -1 &&
                can_go_from_A_to_B(x + dx, y + dy, x, y, map)) {

                if (min == -1) {
                    min = neighbour + 1
                } else {
                    min = Math.min(min, neighbour + 1)
                }
            }
        }
    }

    if (min != -1)
        length_map[y][x] = min;

    return length_map

}

function path_from_starting_point(x_i, y_i, x_f, y_f, map) {
    let length_map = input.split("\n").map(row => new Array(row.length).fill(-1))

    length_map[y_i][x_i] = 0;

    const propagate = () => {
        for (let y in length_map) {
            y = Number.parseInt(y)
            // y = -y + length_map.length - 1
            for (let x in length_map[y]) {
                x = Number.parseInt(x)
                // x = -x + length_map[0].length - 1
                // console.log(x, y)
                length_map = calculate_pos(x, y, map, length_map)

            }
        }
    }

    let MAX = map.length * map[0].length;
    let there_was_change = true;

    while (length_map[y_f][x_f] == -1 && there_was_change) {
        if (MAX == 0) break
        MAX--


        let sum_i = length_map.reduce((sum, row) => sum + row.reduce((sum, elm) => sum + Math.abs(elm), 0), 0)
        there_was_change = propagate()
        let sum_f = length_map.reduce((sum, row) => sum + row.reduce((sum, elm) => sum + Math.abs(elm), 0), 0)

        there_was_change = sum_i != sum_f
    }

    // console.log(length_map.map(row => row.map(n => n.toString(10).padStart(3, " "))).map(row => row.join("")).join("\n"))
    // require("fs").writeFileSync("kk.csv", length_map.map(row => row.join(",")).join("\n"))

    return length_map[y_f][x_f]

}

function main(input) {
    let x_i_list = [];
    let y_i_list = [];



    let x_f = 0;
    let y_f = 0;

    let map = input.split("\n").map((row, y) => {

        return row.split("").map((c, x) => {
            if (c == "S") {
                x_i_list.push(x);
                y_i_list.push(y);
                return char("a");

            }
            if (c == "E") {
                x_f = x;
                y_f = y;
                return char("z");
            }
            if (c == "a") {
                x_i_list.push(x);
                y_i_list.push(y);
            }

            return char(c);
        })

    })

    // console.log(x_i, y_i, x_f, y_f)

    let min = input.length;

    for (const i in x_i_list) {
        // console.log(i + "/" + x_i_list.length)
        let x_i = x_i_list[i];
        let y_i = y_i_list[i];

        let ret = path_from_starting_point(x_i, y_i, x_f, y_f, map)
        console.log(ret)
        if (ret != -1)
            min = Math.min(min, ret)
    }
    console.log(min)


}


let input = ``

// input = `Sabqponm
// abcryxxl
// accszExk
// acctuvwj
// abdefghi`

main(input);